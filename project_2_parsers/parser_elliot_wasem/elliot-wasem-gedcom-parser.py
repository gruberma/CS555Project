...
def main():
    ...
    for line in lines:
        ...
        if not is_level_zero_tag(line_tokens) and not is_level_one_tag(line_tokens) and not is_level_two_tag(line_tokens):
    ...
    # prints each family entry, and adds husband/wife names if they are not there.
    # May not be necessary to check again, but I'm too tired to test and find out
    for family in family_list:
        if family.get("HUSBAND NAME") == "NULL_NAME":
            family["HUSBAND NAME"] = lookup_name(family.get("HUSBAND ID"))
        if family.get("WIFE NAME") == "NULL_NAME":
            family["WIFE NAME"] = lookup_name(family.get("WIFE ID"))
...
def parse_famc_fams_husb_wife_chil(tokens):
    ...
    global cur_family
    global cur_individual
    if (tokens[1] == "FAMC" or tokens[1] == "FAMS" or tokens[1] == "HUSB" or tokens[1] == "WIFE" or tokens[1] == "CHIL") and len(tokens) == 3:
        if tokens[1] == "FAMC":
